...
class StoriesService(object):
    ...
    def get_all(self):
        model = []
        recursivePath = os.path.join(self.stories_path, '**/**/*.yml')
    ...
    def update(self, path, model):
        ...
        newPath = os.path.normpath(os.path.join(os.path.dirname(path), model['name']  + ".yml"))
        os.rename(path, newPath)
        return self.get_by_path(newPath)

    def create(self, model):
        path = os.path.normpath(os.path.join(self.stories_path, model['name']  + ".yml"))
        with open(path, "w") as f:
            f.write(model['text'] or '')
    ...
    def get_model_by_name(self, name):
        path = os.path.join(self.stories_path, name  + ".yml")
        return self.get_model_by_path(path)
    

    def add_story(self, intent, utter):
        story = self.get_model_by_name(intent) or StoriesFileModel(name = intent, path = os.path.join(self.stories_path, intent  + ".yml"))
